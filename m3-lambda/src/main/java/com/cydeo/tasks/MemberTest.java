...
public class MemberTest {
    public static void main(String[] args) {
        ...
        Person p2 = new Person("asya", 25, Gender.FEMALE );
        Person p3 = new Person("tom", 15, Gender.MALE );
        Person p4 = new Person("ally", 40, Gender.FEMALE );
        List<Person> personList = Arrays.asList(p1,p2,p3,p4);


        CheckMember checkMember = p -> p.getGender()==Gender.MALE && p.getAge()<=25;

        print(personList, checkMember);

        print(personList,p -> p.getGender()==Gender.MALE && p.getAge()<=25);
